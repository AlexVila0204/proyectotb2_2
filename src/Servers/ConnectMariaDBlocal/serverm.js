...
CHECK(comm IN(0, 1));
 ...
 console.log("entro a makeTriggers");
  try {
    ...
    //ver si existe un evento llamado replicador
    var evento = await connection.query('SHOW EVENTS LIKE "replicador";');
    var palabra = evento[0];

    //si no existe el evento replicador se crea
    if(palabra == undefined){
      var queryEvento = `
CREATE EVENT replicador
    ON SCHEDULE EVERY 1 MINUTE STARTS '2023-12-09 19:56:03'
    ON COMPLETION NOT PRESERVE
    DO BEGIN
        DECLARE id INT;
        DECLARE accion VARCHAR(15);
        DECLARE tabla VARCHAR(300);
        DECLARE atributos VARCHAR(300);
        DECLARE olddatos VARCHAR(300);
        DECLARE newdatos VARCHAR(300);
        DECLARE conn VARCHAR(255);
        DECLARE create_table_query VARCHAR(500);
        DECLARE querybase VARCHAR(500);

        SELECT id_bitacora, evento, tabla, atributos, oldDatos, newDatos
        INTO id, accion, tabla, atributos, olddatos, newdatos
        FROM bitacora
        WHERE replicado = 0 AND tabla = 'Productos'
        LIMIT 1;

        SET conn := 'DRIVER={SQL Server};SERVER=proyecto;DATABASE=localhost;UID=alexvila;PWD=manzana10';

        SET create_table_query := CONCAT('CREATE TABLE temporal ENGINE=CONNECT, TABLE_TYPE=ODBC, TABNAME=\'', tabla, '\' CONNECTION=\'', conn, '\'');
        ...
        CASE accion
            ...
            WHEN 'U' THEN
                SET querybase := CONCAT('UPDATE temporal SET (', atributos, ')=(', newdatos, ') WHERE (', atributos, ')=(', olddatos, ');');
            WHEN 'D' THEN
                SET querybase := CONCAT('DELETE FROM temporal WHERE (', atributos, ')=(', olddatos, ');');
        END CASE;
...
`;
      ...
      var evento = await connection.query(queryEvento);
      console.log('se creo el evento replicador');
    } else {
      console.log('ya existe el evento replicador');
    }
